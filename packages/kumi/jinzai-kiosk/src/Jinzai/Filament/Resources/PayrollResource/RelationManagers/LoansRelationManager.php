...
use Kumi\Jinzai\Actions\CalculateLoanFinalDate;
use Filament\Resources\RelationManagers\RelationManager;
use Kumi\Jinzai\Filament\Resources\PayrollResource\RelationManagers\LoansRelationManager\Actions as TableActions;
                ...
                Forms\Components\Grid::make(1)
                    ->schema([
                        Forms\Components\DatePicker::make('started_at')
                            ->label(__('jinzai::filament/resources/loan.fields.started_at.label'))
                            ->displayFormat('d F Y')
                        ...
                        Forms\Components\DatePicker::make('finalized_at')
                            ->label(__('jinzai::filament/resources/loan.fields.finalized_at.label'))
                            ->disabled()
                            ->displayFormat('d F Y')
                            ->required(),
                    ])->columnSpan(1),
                Forms\Components\Grid::make(1)
                    ->schema([
                        Forms\Components\TextInput::make('loan_amount')
                            ->label(__('jinzai::filament/resources/loan.fields.loan_amount.label'))
                            ->prefix('IDR')
                            ->default(0)
                            ->required()
                            ->reactive()
                        ...
                        Forms\Components\TextInput::make('installment_amount')
                            ->label(__('jinzai::filament/resources/loan.fields.installment_amount.label'))
                            ->prefix('IDR')
                            ->default(0)
                            ->required()
                            ->reactive()
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('started_at')
                    ->label(__('jinzai::filament/resources/loan.columns.started_at.label'))
                    ->formatStateUsing(function (Carbon $state) {
                        return $state->format('d F Y');
                    }),
                Tables\Columns\TextColumn::make('finalized_at')
                    ->label(__('jinzai::filament/resources/loan.columns.finalized_at.label'))
                    ->formatStateUsing(function (Carbon $state) {
                        return $state->format('d F Y');
                    }),
                Tables\Columns\TextColumn::make('loan_amount_formatted')
                    ->label(__('jinzai::filament/resources/loan.columns.loan_amount.label'))
                    ->alignRight()
                    ->extraAttributes(['class' => 'font-mono']),
                Tables\Columns\TextColumn::make('installment_amount_formatted')
                    ->label(__('jinzai::filament/resources/loan.columns.installment_amount.label'))
                    ->alignRight()
                    ->extraAttributes(['class' => 'font-mono']),
                Tables\Columns\BadgeColumn::make('status')
                    ->label(__('jinzai::filament/resources/loan.columns.status.label'))
                    ->enum([
                        true => __('jinzai::filament/resources/loan.columns.status.options.approved'),
                        false => __('jinzai::filament/resources/loan.columns.status.options.pending'),
                    ])
                    ->colors([
